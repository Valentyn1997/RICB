...
class MSM(BaseNet):
    ...
    def __init__(self, args: DictConfig = None, mlflow_logger: MLFlowLogger = None, **kwargs):
        ...
        if self.network_type == 'snet':
            ...
            self.cond_spline_nn = torch.nn.ModuleList([DenseNN(self.dim_repr, [self.dim_hid1],
            ...
            self.cond_spline_transform = torch.nn.ModuleList([T.ConditionalSpline(cond_spline_nn, 1, order='quadratic',
                                                                                  count_bins=self.count_bins,
                                                                                  bound=self.bound).to(self.device)
                                                              for cond_spline_nn in self.cond_spline_nn])
            self.cond_flow_dist = [dist.ConditionalTransformedDistribution(self.cond_base_dist, [cond_spline_transform])
                                   for (cond_spline_transform) in self.cond_spline_transform]
        else:
    ...
    def prepare_train_data(self, data_dict: dict):
        ...
        mu_pred0_repr = torch.tensor(data_dict['mu_pred0_repr']).float()
        mu_pred1_repr = torch.tensor(data_dict['mu_pred1_repr']).float()
    ...
    def fit(self, train_data_dict: dict, log: bool):
        ...
        for step in tqdm(range(self.num_epochs)) if log else range(self.num_epochs):
            ...
            loss, log_dict = self.forward_train(noised_repr_f, treat_f, noised_out_f, mu_pred0_repr, mu_pred1_repr,
                                                prefix='train')
            loss.backward()
    ...
    def forward_train(self, repr_f, treat_f, out_f, mu_pred0_repr, mu_pred1_repr, prefix='train'):
        ...
        if self.network_type == 'tnet':
            log_prob = torch.stack([(treat_f == t) * self.cond_flow_dist[int(t)].condition(repr_f).log_prob(out_f)
                                    for t in self.treat_options], dim=0).sum(0)
        ...
        for i in range(repr_f.shape[0] // batch_size + 1):
            ...
            with torch.no_grad():
                for t in self.treat_options:
                    if self.network_type == 'tnet':
                        out_sample = self.cond_flow_dist[int(t)].condition(repr_f_batch).sample((n_samples,
                                                                                                 repr_f_batch.shape[0]))
                    elif self.network_type == 'snet':
    ...
    def evaluate_policy(self, data_dict: dict, delta: float, log: bool, prefix: str):
        ...
        # policy_val_deferral = (out_pot1[~deferral_mask] * (lb[~deferral_mask] > 0.0) +
        #      out_pot0[~deferral_mask] * (ub[~deferral_mask] < 0.0)).mean()
    ...
    def calculate_gammas(data_dict, reference_data_dict=None, clip_prop=0.0, set_nan_to=1000.0, delta=0.05):
        ...
        mask = (prop_pred_cov > clip_prop) & (prop_pred_repr > clip_prop) & \
               (1.0 - prop_pred_cov > clip_prop) & (1.0 - prop_pred_repr > clip_prop)
        if mask.sum() > 0:
        ...
        if reference_data_dict is None:
            ...
            reference_gammas = MSM.calculate_gammas(reference_data_dict, reference_data_dict=None, clip_prop=clip_prop,
                                                    set_nan_to=set_nan_to)
        ...
        for i in range(repr_f.shape[0] // batch_size + 1):
            ...
            with torch.no_grad():
                for t in self.treat_options:
                    ...
                    s_minus = 1.0 / ((1.0 - gammas_batch) * q_t + gammas_batch)
                    s_plus = 1.0 / ((1.0 - 1.0 / gammas_batch) * q_t + 1.0 / gammas_batch)
                    ...
                    q_index_minus, q_index_plus = torch.clamp(q_index_minus, 0, n_samples - 1), \
                        torch.clamp(q_index_plus, 0, n_samples - 1)

                    if self.network_type == 'tnet':
                        out_sample = self.cond_flow_dist[int(t)].condition(repr_f_batch).sample((n_samples,
                                                                                                 repr_f_batch.shape[0])).squeeze()
                    elif self.network_type == 'snet':
                        context = torch.cat([repr_f_batch, t * torch.ones((repr_f_batch.shape[0], 1))], dim=1)
                    ...
                    mu_minus[large_gamma_mask] = out_sample[0, large_gamma_mask]
                    mu_plus[large_gamma_mask] = out_sample[-1, large_gamma_mask]
    ...
    def get_coverage(self, data_dict):
        ...
        pred_cover0 = ((data_dict['mu_pred0_repr'].squeeze() > data_dict['mu_pred0_PSM_bounds'][0]) &
                       (data_dict['mu_pred0_repr'].squeeze() < data_dict['mu_pred0_PSM_bounds'][1])).mean()
        pred_cover1 = ((data_dict['mu_pred1_repr'].squeeze() > data_dict['mu_pred1_PSM_bounds'][0]) &
                       (data_dict['mu_pred1_repr'].squeeze() < data_dict['mu_pred1_PSM_bounds'][1])).mean()
        pred_cover = 0.5 * (pred_cover0 + pred_cover1)

        mu0 = self.out_scaler.transform(data_dict['mu0'].reshape(-1, 1))
        mu1 = self.out_scaler.transform(data_dict['mu1'].reshape(-1, 1))

        gt_cover0 = ((mu0.squeeze() > data_dict['mu_pred0_PSM_bounds'][0]) &
                     (mu0.squeeze() < data_dict['mu_pred0_PSM_bounds'][1])).mean()
        gt_cover1 = ((mu1.squeeze() > data_dict['mu_pred1_PSM_bounds'][0]) &
                     (mu1.squeeze() < data_dict['mu_pred1_PSM_bounds'][1])).mean()
